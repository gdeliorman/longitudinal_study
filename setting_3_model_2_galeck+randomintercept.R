...
for (i in 1:length(rho_delta_1)) {
  for (j in 1:length(rho_delta_2)) {
    ...
    if (is.positive.definite(V_1)==TRUE & is.positive.definite(D_1)==TRUE ) {
      ...
      write(as.numeric(rh_square_check),"~/Downloads/RH_square_check_setting3_model2.txt",  append=TRUE, sep = "\t")
...
RH_square_check<-read.delim(file= '~/Downloads/RH_square_check_setting3_model2.txt', header = FALSE, sep = "\t", dec = ".")
...
hist( as.numeric(RH_square_check$V1), main="", xlab=bquote("R"[Lambda]^2),  breaks = 15, labels = TRUE,ylim = c(0,1200000))
grid()
box()
