...
for (i in 1:length(rho_delta_1)) {
  for (j in 1:length(rho_delta_2)) {
    ...
    if (is.positive.definite(V_1)==TRUE & is.positive.definite(D_1)==TRUE ) {
      ...
      write(as.numeric(rh_square_check),"~/Downloads/RH_square_check_set4_model2_22may.txt",  append=TRUE, sep = "\t")
...
RH_square_check<-read.delim(file= '~/Downloads/RH_square_check_set4_model2_22may.txt', header = FALSE, sep = "\t", dec = ".")
