##setting 1 model 3 type=VC using initial values from experimental and placebo groups

library(matrixcalc)

V<- matrix(c(50.2520,0,0,0,
             0,50.2520,0,0,
             0,0,50.2520, 0,
             0,0,0,50.2520), nrow = 4, ncol = 4)

corr_V<- cov2cor(V)
cor_matrix<-diag(5)


##V and corr are constant.
...
un11<- D[1,1] <- 38.9493
un22<- D[3,3] <- 98.3507
un31<- D[5,1] <- D[1,5] <- 93.8709
un33<- D[5,5] <- 137.49
un42<- D[7,3] <- D[3,7] <- 171.12
un44<- D[7,7] <- 273.30
un51<- D[1,2] <- D[2,1] <- -2.3488
un53<- D[2,5] <- D[5,2] <- -13.0340
un55<- D[2,2] <- 2.3934
un62<- D[3,4] <- D[4,3] <- 0.8413
un64<- D[4,7] <- D[7,4] <- 1.5195
un66<- D[4,4] <- 4.1359
un71<- D[1,6] <- D[6,1] <- -12.6741
un73<- D[5,6] <- D[6,5] <- -17.0046
un75<- D[2,6] <- D[6,2] <- 7.9105
un77<- D[6,6] <- 12.4668
un82<- D[3,8] <- D[8,3] <- -1.3306
un84<- D[7,8] <- D[8,7] <- 8.3980
un86<- D[4,8] <- D[8,4] <- 7.6536
un88<- D[8,8] <- 9.4220
...
##note in sas: D is not positive definite.
##in R double check: D is not positive definite
...
##calculate R_H^2
 for(j in 1:length(complete_D)){
    ...
    u_11<- as.numeric(V[1,1]+ V[2,2]- V[1,2])
    u_22<- as.numeric(V[3,3]+ V[4,4]- V[3,4])
    u_12<- as.numeric(V[1,3]+ V[2,4]- V[2,3]-V[1,4] )
    ...
    delta_sigma_2<- round(delta_sigma, digits = 4)
