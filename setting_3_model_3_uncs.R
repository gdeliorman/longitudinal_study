##setting 3 model 3 type=un@ar(1) p=6 (6 time points)

library(matrixcalc)

V<- matrix(c(35.0782,NA,53.9209,NA,
             NA,32.6412,NA,51.4275,
             53.9209,NA,95.0380, NA,
             NA,51.4275,NA,91.8196), nrow = 4, ncol = 4)
...
un11<- D[1,1] <- 120.54
un22<- D[3,3] <- 87.1524
un31<- D[5,1] <- D[1,5] <- 184.00
un33<- D[5,5] <- 324.53
un42<- D[7,3] <- D[3,7] <- 141.16
un44<- D[7,7] <- 272.21
un51<- D[1,2] <- D[2,1] <- -1.3282
un53<- D[2,5] <- D[5,2] <- -2.5929
un55<- D[2,2] <- 1.6895
un62<- D[3,4] <- D[4,3] <- -3.1026
un64<- D[4,7] <- D[7,4] <- -4.5776
un66<- D[4,4] <- 2.2850
un71<- D[1,6] <- D[6,1] <- 0.9976
un73<- D[5,6] <- D[6,5] <- -0.6961
un75<- D[2,6] <- D[6,2] <- 3.0807
un77<- D[6,6] <- 5.9252
un82<- D[3,8] <- D[8,3] <- -3.8834
un84<- D[7,8] <- D[8,7] <- -7.1991
un86<- D[4,8] <- D[8,4] <- 3.9908
un88<- D[8,8] <- 7.3546
...
#for(i in 955:length(complete_V)){
  for(j in 1:length(complete_D)){
    ...
    if ( is.positive.definite(delta_sigma_2)==TRUE & is.positive.definite(delta_u)==TRUE){
      ...
      write(mean(as.numeric(rh_square)), file="~/Downloads/rh_square_2may_uncs_setting1.txt", append=TRUE)
...
RH_square_2_txt<-read.delim(file= '~/Downloads/rh_square_2may_uncs_setting1.txt', header = FALSE, sep = "\t", dec = ".")
